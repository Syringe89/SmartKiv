                                        ...
                                        float *max_angle, float *max_voltage)
{
    ...
    // Инициализируем LEDC в среднем положении
    init_ret = ledc_init(servo_calculate_duty(current_angle));
    if (init_ret != ESP_OK)
    {
...
esp_err_t servo_calibration_run(servo_calibration_config_t *config, servo_calibration_result_t *result)
{
    ...
    if (ret != ESP_OK)
    {
        ...
        servo_position_reader_deinit(); // Деинициализация АЦП в случае ошибки
        return ret;
    }
        ...
        fabsf(result->min_voltage_mv - result->max_voltage_mv) < config->threshold_voltage_mv * 10.0f)
    {
        ...
        servo_position_reader_deinit(); // Деинициализация АЦП в случае ошибки валидации
